...
static mnist_dataset_t *load_split(const char *root, const char *split) {
    // Charge le cache binaire si present pour limiter le temps de preparation des donnees.
    char cache_path[512];
...
int main(int argc, char **argv) {
    ...
    // Variante souvent plus stable sur MNIST: ReLU + Softmax + CCE avec Adam.
    ...
    printf("Config entrainement: hidden=relu, output=softmax, loss=categorical_cross_entropy, optimizer=adam\n");
    nn_network_train(network, train, 0.001f, 10);

    float accuracy = nn_network_evaluate(network, test);
    printf("Precision test: %.2f%%\n", accuracy * 100.0f);

    nn_network_save(network, "mnist_relu_softmax_adam_cce.bin");
